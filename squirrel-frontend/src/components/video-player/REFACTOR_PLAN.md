...
### Phase 5: 回归验证与文档 [1-2周] ✅ 已收敛
**目标**: 在当前仓库不新增前端测试的前提下，保留播放器专项回归清单和文档入口

> 仓库约定：前端不再维护、不新增、不运行测试。因此本阶段不引入 Vitest、组件测试或 E2E 测试。

- [x] 5.1 手工回归清单
  - [x] 基础加载：普通 MP4、HLS、DASH 能进入可播放状态，错误态能显示
  - [x] 播放源切换：从列表进入播放页、切换视频、返回后重新进入不会复用旧播放源
  - [x] 质量降级：HLS/DASH 清晰度切换失败时能降级并保留可播放源
  - [x] 字幕渲染：VTT/SRT 字幕可加载、显示、切换和关闭
  - [x] 快捷键：播放/暂停、前进/后退、音量、全屏快捷键不影响输入框
  - [x] 销毁清理：离开播放页后媒体实例、插件监听器、定时器和全局事件被释放
- [x] 5.2 文档入口
  - [x] 当前文件保留核心 API、Vue 组件 API、插件 API 示例
  - [x] 手工回归清单作为后续播放器改动的验收入口
...
| M5: 播放器回归清单完成 | Week 8 | ✅ 已完成 |
| M6: 文档入口完成 | Week 9 | ✅ 已完成 |
...
### 2026-06-07
- Phase 5 按仓库约定收敛为手工回归清单，不新增前端测试框架或测试用例
- 补充播放器基础加载、播放源切换、质量降级、字幕、快捷键、销毁清理验收项
