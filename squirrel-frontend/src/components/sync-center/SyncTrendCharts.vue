  ...
  <div class="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-border-primary bg-bg-secondary">
    <div class="border-b border-border-primary px-3 py-2.5">
      <div class="flex items-center justify-between gap-3">
        <div>
          <div class="text-xs font-semibold tracking-[0.14em] text-text-tertiary">趋势切片</div>
          <div class="mt-1 text-2xs text-text-muted">范围 {{ range.toUpperCase() }} · 时间桶 {{ series.length }}</div>
        </div>
      ...
      <div v-if="!embedded" class="mt-3 grid grid-cols-1 gap-3 xl:grid-cols-[1.1fr_1fr_1fr_1fr]">
        ...
        <input :value="filters.mode" type="text" placeholder="模式" class="w-full rounded-lg border border-border-primary bg-bg-primary px-3 py-2 text-sm text-text-primary" @input="emitInput('mode', $event)" />
        <input :value="filters.trigger" type="text" placeholder="触发方式" class="w-full rounded-lg border border-border-primary bg-bg-primary px-3 py-2 text-sm text-text-primary" @input="emitInput('trigger', $event)" />
      </div>
    </div>
        ...
        <div class="flex-1 overflow-auto">
          <table class="w-full" :class="embedded ? 'min-w-[460px]' : 'min-w-[560px]'">
            ...
            <tbody class="divide-y divide-border-primary">
              <tr v-for="point in displaySeries" :key="`${point.bucket_time}-${point.site}-${point.sync_mode}-${point.trigger}`" class="text-2xs text-text-secondary">
                <td class="px-3 py-2 text-text-primary">{{ point.bucket_time }}</td>
      ...
      <div class="flex min-h-0 flex-col overflow-hidden rounded-xl border border-border-primary bg-bg-primary">
        ...
        <div class="flex-1 overflow-auto">
          <table class="w-full" :class="embedded ? 'min-w-[360px]' : 'min-w-[320px]'">
            ...
            <tbody class="divide-y divide-border-primary">
              <tr v-for="item in displaySites" :key="item.site" class="text-2xs text-text-secondary">
                <td class="px-3 py-2 text-text-primary">{{ item.site }}</td>
