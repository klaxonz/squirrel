<template>
  <div class="grid grid-cols-2 xl:grid-cols-5 gap-3">
    <button
      ...
      :key="card.key"
      type="button"
      class="text-left bg-bg-secondary border rounded-2xl p-4 transition-all"
      :class="card.clickable
        ? (selectedStatus === card.status
          ? 'border-border-hover bg-bg-elevated shadow-sm'
    ...
    >
      <div class="flex items-start justify-between gap-3">
        <div>
          <p class="text-xs text-text-tertiary">{{ card.label }}</p>
          <p class="text-2xl font-semibold text-text-primary mt-2">{{ card.value }}</p>
        </div>
        <span class="h-2.5 w-2.5 rounded-full" :class="card.dotClass"></span>
      </div>
      <p class="text-2xs text-text-muted mt-3">{{ card.description }}</p>
    </button>
  </div>
</template>
