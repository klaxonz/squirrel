...
import type { VideoSubtitle, VideoInfo } from '../types/video-player'

export interface UseSubtitlesOptions {
  store: PlayerStore
  videoRef: Ref<HTMLVideoElement | null> | ComputedRef<HTMLVideoElement | null>
  props: {
    video?: VideoInfo & { subtitles?: VideoSubtitle[] }
  }
}
...
export interface UseSubtitlesReturn {
  ...
  ensureSubtitlesOnMetadata: () => void
  nextSubtitle: () => void
}

export default function useSubtitles({ store, videoRef, props }: UseSubtitlesOptions): UseSubtitlesReturn {
  const getTrack = (): TextTrack | null => {
    return videoRef?.value?.textTracks?.[0] || null
  }

  const applyCuePosition = (track: TextTrack): void => {
    try {
  ...
  const ensureVideoCssClass = (): void => {
  ...
  const loadSubtitle = async (subtitle: VideoSubtitle): Promise<void> => {
    try {
      ...
      if (!track) {
        ...
        const langCode = (subtitle as any).srclang || 'zh'
  ...
  const setSubtitle = (subtitle: VideoSubtitle | null): void => {
  ...
  const ensureSubtitlesOnMetadata = (): void => {
    if (!videoRef?.value) return
