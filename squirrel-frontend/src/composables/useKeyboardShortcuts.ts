...
import type { PlayerStore } from './usePlayerContext'
...
export default function useKeyboardShortcuts(
  ...
  const keyboardFeedbackTimer: Ref<ReturnType<typeof setTimeout> | null> = ref(null)

  /**
   * 显示键盘反馈信息
   */
  const showKeyboardFeedback = (message: string): void => {
   ...
   * 执行键盘动作
   */
  const executeKeyboardAction = (action: string, args: any[] = []): void => {
    const {
      togglePlay,
      skipForward,
      skipBackward,
    ...
    switch (action) {
      ...
      case 'nextSubtitle':
        if (typeof nextSubtitle === 'function') {
          nextSubtitle()
        }
        break
 ...
 * 获取所有键盘快捷键列表（用于帮助面板）
 */
export function getKeyboardShortcutsList(): Array<{ key: string; description: string }> {
  return Object.entries(keyboardShortcuts).map(([key, shortcut]) => ({
    key: key === ' ' ? 'Space' : key,
