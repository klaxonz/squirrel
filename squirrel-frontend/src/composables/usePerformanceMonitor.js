export default function usePerformanceMonitor({ hlsRef, videoRef, externalPerformanceState }) {
  const MEMORY_CLEANUP_INTERVAL = 30000;
  const BANDWIDTH_SAMPLE_SIZE = 5;

  const performanceState = externalPerformanceState || {
  ...
  const monitorNetworkSpeed = () => {
    ...
    for (let i = 0; i < buffered.length; i++) {
      ...
      const estimatedBitrate = 5 * 1024 * 1024; // 5Mbps
    ...
    if (videoRef?.value && videoRef.value.buffered.length > 0) {
      ...
      for (let i = 0; i < buffered.length; i++) {
        if (buffered.end(i) < currentTime - 30) {
          // hint for GC, nothing to do explicitly
        }
      }
    }
