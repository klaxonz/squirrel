...
test('video player pauses the previous source immediately when the parent clears the source during a switch', async () => {
  ...
  assert.match(source, /if \(!s\) \{\s*shouldResumeAfterSourceSwap\.value = isPlaying\.value\s*pause\(\)\s*return\s*\}/)
  assert.match(source, /loadSource\(s\)[\s\S]*?if \(shouldResumeAfterSourceSwap\.value\) \{\s*shouldResumeAfterSourceSwap\.value = false\s*void play\(\)\s*\}/)
