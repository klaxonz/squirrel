...
test('desktop youporn login verifies the session page', async () => {
  ...
  assert.match(source, /const buildYouPornLoginWindowStatus = async \(profile, loginWindow\) => \{[\s\S]*?loginWindow\.webContents\.executeJavaScript/)
  assert.match(source, /const profileLink = document\.querySelector\('#js_mainMenu \.user-item a\[href\^="\/users\/"\]'\)/)
  assert.match(source, /if \(profile\.siteName === 'youporn'\) \{[\s\S]*?buildYouPornLoginWindowStatus\(profile, loginWindow\)/)
