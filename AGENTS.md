...
<!-- CODEX_MULTI_AGENT_BEGIN -->
## Codex Multi-agent 协作规范（默认只读，授权可写）

### 默认策略
- 子 Agent 默认只读；只有看到 WRITE_SCOPE: ... 才允许写入。
- 默认并发上限：5（只有在可整合性仍然很好时才考虑更高并发）。

### 并发默认启动组合（=3）
- 当任务适合多 Agent 时，优先并行启动 Explorer(1) + Verifier(1) + Scribe(1)。
- Worker/Integrator 只有在主 Agent 明确给出 WRITE_SCOPE 后才允许启动并写入。

### 角色库（Role Catalog）
- Explorer（只读）：检索定位（文件/符号/调用链/配置入口）。
- Verifier（只读）：最短验证路径（复现/测试命令/回归面）。
- Test-Worker（可写，需授权）：只写/改测试，不改实现（除非允许）。
- Fix-Worker（可写，需授权）：只改实现，尽量不动测试（除非允许）。
- Integrator（可写，需授权）：冲突/风格/关键验证/最小修补，禁止顺手重构。
- Scribe（只读）：变更摘要/发布说明/风险与回滚点/验证记录。

### 写入授权协议（主 Agent 必须明确写出来）
- ROLE: <...>
- WRITE_SCOPE: <path1>, <path2>, ...（Worker/Integrator 必填）
可选：WRITE_LIMITS: ...、VERIFY: ...

没有 WRITE_SCOPE 时，即使是 Worker/Integrator 也必须只读并输出建议。
