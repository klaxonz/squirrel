...
async def tasks_progress(task_ids: str, session: Session = Depends(get_session)):
    task_service = TaskService(session)
...
async def new_task_notification(latest_task_id: int = Query(default=0), session: Session = Depends(get_session)):
    task_service = TaskService(session)
