...
def fetch_with_cloudflare_bypass(
    ...
    try:
        ...
        while total_wait < config.max_wait_time and verification_count < config.max_verification_attempts:
            ...
            logger.debug(
                f"Cloudflare verification attempt {verification_count}/{config.max_verification_attempts} for {url}")
            try:
                driver.detect_and_bypass_cloudflare()
            except Exception as e:
                logger.warning(f"Cloudflare verification failed for {url}", e)
