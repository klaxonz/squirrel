...
PLUGINS_ROOT = REPO_ROOT / 'squirrel-plugins'
...
def _stub_crawl_module():
    ...
    @dataclass
    class PluginCapability(_BaseModel):
        name: str
        description: str = ''
    ...
    @dataclass
    class PluginPermission(_BaseModel):
        name: str
        description: str = ''
        required: bool = True
        scope: str | None = None
    ...
    @dataclass
    class PluginSiteManifest(_BaseModel):
        site_name: str
    ...
    @dataclass
    class PluginHealthStatus(_BaseModel):
        healthy: bool
        status: str = 'unknown'
        message: str = ''
    ...
    def create_plugin_runtime(*, manifest, **_kwargs):
    ...
    crawl_module.PluginCapability = PluginCapability
    crawl_module.PluginHealthStatus = PluginHealthStatus
    crawl_module.PluginManifest = PluginManifest
    crawl_module.PluginPermission = PluginPermission
    crawl_module.PluginSiteManifest = PluginSiteManifest
    ...
    crawl_module.create_plugin_runtime = create_plugin_runtime
...
class RuntimeV2PackageTests(unittest.TestCase):
    ...
    def test_runtime_manifest_matches_plugin_runtime_json(self):
        for plugin_name, spec in PLUGIN_SPECS.items():
            with self.subTest(plugin=plugin_name):
                plugin_dir = PLUGINS_ROOT / plugin_name
                ...
                self.assertEqual(actual_manifest['plugin_id'], expected_manifest['plugin_id'])
