...
class ConnectivityTestRequest(BaseModel):
    ...
    timeout: int | None = Field(10, description="超时时间（秒），默认10秒", ge=1, le=60)
    follow_redirects: bool | None = Field(True, description="是否跟随重定向")
...
class ConnectivityTestResponse(BaseModel):
    ...
    final_url: str | None = Field(None, description="最终访问的URL（如果有重定向）")
    error_message: str | None = Field(None, description="错误信息")
    dns_resolved: bool | None = Field(None, description="DNS是否解析成功")
    ip_address: str | None = Field(None, description="解析到的IP地址")
    headers: dict[str, str] | None = Field(None, description="响应头信息")
