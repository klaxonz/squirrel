...
@queue_handler("video_extract_batch")
def process_batch_video_extract(message: Dict[str, Any]):
    """批量视频提取处理器
    ...
    try:
        ...
        only_extract = message.get('only_extract', False)

        logger.info(f"开始批量视频提取: {len(urls)} 个视频")

        results = []
        for url in urls:
            try:
                extract_message = {
                    "body": VideoExtractDto(
                        url=url,
                ...
                # 使用路由发送
                queue_name = MessageRouter.send_with_routing("video_extract", extract_message)
                results.append({"url": url, "queue": queue_name, "status": "queued"})

            except Exception as e:
                logger.error(f"批量提取中单个URL失败: {url}, error: {e}")
                results.append({"url": url, "status": "failed", "error": str(e)})

        logger.info(f"批量视频提取完成: 成功 {len([r for r in results if r['status'] == 'queued'])} 个")

    except Exception as e:
        logger.error(f"批量视频提取失败: {e}", exc_info=True)
        raise


@queue_handler("video_extract_priority_*")
def process_priority_video_extract(message: Dict[str, Any], queue_name: str):
    priority = 'normal'  # 默认优先级
    try:
        # 解析优先级
        priority = queue_name.split('_')[-1] if '_' in queue_name else 'normal'

        logger.info(f"开始处理优先级视频提取: priority={priority}")
        ...
        message['_max_retries'] = max_retries

        process_video_extract(message, queue_name)

    except Exception as e:
        logger.error(f"优先级视频提取失败: priority={priority}, error={e}", exc_info=True)
        raise
