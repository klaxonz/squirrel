...
def load_existing_batch_records(batch_name: str) -> set[str]:
    with get_session() as session:
        rows = session.execute(
            select(VideoThumbnailLocalIndex.filename).where(
...
def backfill_thumbnail_local_index(
    ...
    pending_batch_dirs = [batch_dir for batch_dir in batch_dirs if batch_dir.name not in completed_batches]
    processed = 0
    completed_lock = Lock()
    ...
    def _process_batch(batch_dir: Path) -> tuple[str, int, int, str]:
        ...
        records_to_upsert = [record for record in batch_records if record[2] not in existing_filenames]
    ...
    with ThreadPoolExecutor(max_workers=max(workers, 1)) as executor:
        futures = [executor.submit(_process_batch, batch_dir) for batch_dir in pending_batch_dirs]
