...
from core import cookie_config
...
def test_write_cookie_text_file_replaces_existing_content(tmp_path):
    ...
    cookie_config.write_cookie_text_file(target, 'new-value\n')
...
def test_write_cookie_text_file_keeps_original_content_when_replace_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cookie_config.os, 'replace', _raise_replace)

    with pytest.raises(OSError, match='replace failed'):
        cookie_config.write_cookie_text_file(target, 'new-value\n')

    monkeypatch.setattr(cookie_config.os, 'replace', original_replace)
