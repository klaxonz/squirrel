...
from routes.user.dependencies import get_user_service
...
class FakeUserService:
    def __init__(self, authenticate_impl=None, update_password_impl=None, rotate_token_version_impl=None):
        self._authenticate_impl = authenticate_impl or (lambda email, password: (_make_user(email=email), object()))
        self._update_password_impl = update_password_impl
        self._rotate_token_version_impl = rotate_token_version_impl

    def authenticate(self, email, password):
        return self._authenticate_impl(email, password)

    def update_password(self, user_id, current_password, new_password):
        return self._update_password_impl(user_id, current_password, new_password)

    def rotate_token_version(self, user_id):
        return self._rotate_token_version_impl(user_id)


def _build_app(
    monkeypatch,
    current_user=None,
    authenticate_impl=None,
    create_access_token_impl=None,
    update_password_impl=None,
