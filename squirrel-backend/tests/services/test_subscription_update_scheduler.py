...
def _patch_postgres(session_factory):
    ...
    with (
        patch.object(database, 'register_after_commit', lambda session, callback: None),
...
def test_schedule_one_creates_full_sync_crawl_task(engine, session_factory, sched):
    ...
    with (
        patch('infrastructure.site_catalog.catalog.SiteCatalog.is_site_enabled', return_value=True),
...
def test_enqueue_all_active_includes_active_subscriptions_without_sync_state(engine, session_factory, sched):
    with Session(engine, expire_on_commit=False) as session:
        session.add_all(
            [
                Subscription(
                    id=1, type='CHANNEL', name='due-no-state', url='https://space.bilibili.com/1', is_deleted=False
                ),
                Subscription(
                    id=2,
                    type='CHANNEL',
                    name='due-existing-state',
                    url='https://space.bilibili.com/2',
                    is_deleted=False,
                ),
                Subscription(
                    id=3, type='CHANNEL', name='not-due', url='https://space.bilibili.com/3', is_deleted=False
                ),
                Subscription(
                    id=4, type='CHANNEL', name='no-subscriber', url='https://space.bilibili.com/4', is_deleted=False
                ),
    ...
    with patch.object(
        ...
        lambda self, subscription_id, url, trigger, mode: (
            scheduled_calls.append((subscription_id, url, trigger, mode)) or SimpleNamespace(status='queued')
        ),
    ):
...
def test_enqueue_all_active_counts_failed_results(engine, session_factory, sched):
    ...
    with (
        patch.object(ssss, 'get_sync_state', lambda subscription_id, mode: None),
        patch.object(
            SubscriptionScheduler,
            'schedule_one',
    ...
    with (
        ...
        with Session(engine, expire_on_commit=False) as session:
            session.add_all(
                [
                    Subscription(
                        id=1, type='CHANNEL', name='due-1', url='https://space.bilibili.com/1', is_deleted=False
                    ),
                    Subscription(
                        id=2, type='CHANNEL', name='due-2', url='https://space.bilibili.com/2', is_deleted=False
                    ),
                ],
            )
            session.add_all(
