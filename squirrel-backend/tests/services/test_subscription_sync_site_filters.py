...
def test_list_sync_center_queued_items_follow_real_task_queue_order(monkeypatch):
    ...
    assert [item.subscription_id for item in result.data] == [1, 2]
    assert [item.queue_position for item in result.data] == [1, 2]


def test_list_sync_center_queued_items_only_include_dispatch_candidates(monkeypatch):
    engine = _setup_test_env(monkeypatch)
    _seed_sync_projection(engine)
    ...
    with Session(engine, expire_on_commit=False) as session:
        ...
        subscription_projection.current_status = 'queued'
        subscription_projection.current_phase = 'queued'
        subscription_projection.updated_at = datetime(2024, 1, 1, 0, 0, 0)

        session.add_all([
            Subscription(
                id=2,
                type='CHANNEL',
                name='YouTube Backlog',
                url='https://www.youtube.com/channel/demo-2',
                avatar=None,
                description=None,
                total_videos=0,
            ...
            Subscription(
                ...
                type='CHANNEL',
                name='Bilibili Head',
                url='https://space.bilibili.com/demo-3',
                avatar=None,
                description=None,
                total_videos=0,
                is_deleted=False,
        ...
        session.add_all([
            ...
            SubscriptionSyncRunProjection(
                ...
                status='queued',
                current_phase='queued',
                queued_at=datetime(2024, 1, 1, 0, 1, 0),
                started_at=None,
                finished_at=None,
                duration_ms=0,
                failure_count=0,
                ...
                pending_video_count=0,
                last_event_seq_no=1,
                last_event_at=datetime(2024, 1, 1, 0, 1, 0),
            ...
            SubscriptionSyncRunProjection(
                ...
                sync_mode='incremental',
                trigger='manual',
                request_id='req-queued-c',
                trace_id='trace-queued-c',
                status='queued',
                current_phase='queued',
                queued_at=datetime(2024, 1, 1, 0, 2, 0),
                started_at=None,
                finished_at=None,
                duration_ms=0,
                failure_count=0,
                ...
                pending_video_count=0,
                last_event_seq_no=1,
                last_event_at=datetime(2024, 1, 1, 0, 2, 0),
            ...
            SubscriptionSyncSubscriptionProjection(
                ...
                current_status='queued',
                current_phase='queued',
                last_sync_at=datetime(2024, 1, 1, 0, 1, 0),
                last_success_at=datetime(2024, 1, 1, 0, 1, 0),
                next_sync_at=datetime(2024, 1, 1, 2, 0, 0),
                last_error_message=None,
                pending_video_count=0,
                failure_streak=0,
            ...
            SubscriptionSyncSubscriptionProjection(
                subscription_id=3,
                latest_run_id='run-queued-c',
                current_status='queued',
                current_phase='queued',
                last_sync_at=datetime(2024, 1, 1, 0, 2, 0),
                last_success_at=datetime(2024, 1, 1, 0, 2, 0),
                next_sync_at=datetime(2024, 1, 1, 2, 0, 0),
                last_error_message=None,
                pending_video_count=0,
                failure_streak=0,
            ...
            CrawlTask(
                ...
                priority='normal',
                payload={'sync_state_id': 12},
                next_run_at=datetime(2024, 1, 1, 0, 1, 0),
                created_at=datetime(2024, 1, 1, 0, 1, 0),
                updated_at=datetime(2024, 1, 1, 0, 1, 0),
            ),
            CrawlTask(
                ...
                status='pending',
                priority='normal',
                payload={'sync_state_id': 13},
                next_run_at=datetime(2024, 1, 1, 0, 2, 0),
                created_at=datetime(2024, 1, 1, 0, 2, 0),
                updated_at=datetime(2024, 1, 1, 0, 2, 0),
            ),
        ])
    ...
    assert [item.subscription_id for item in result.data] == [1, 3, 2]
    assert [item.queue_position for item in result.data] == [1, 2, 3]
