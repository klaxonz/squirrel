...
def test_resolve_cookie_file_for_url_maps_youtube_media_domains(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda slug: cookie_file if slug == 'youtube' else tmp_path / 'missing.txt',
    )

    assert cookie.resolve_cookie_file_for_url('https://rr4---sn-a5meknzl.googlevideo.com/videoplayback') == str(
        cookie_file
    )
