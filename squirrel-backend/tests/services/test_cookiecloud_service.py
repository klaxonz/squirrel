...
def test_sync_cookiecloud_to_site_files_uses_safe_cookie_file_writer(tmp_path):
    ...
    with (
        ...
        patch(
            'infrastructure.site_catalog.cookiecloud.get_effective_site_catalog',
            return_value={
                'youtube': {
                    'domains': ['youtube.com'],
                },
        ...
        patch(
            'infrastructure.site_catalog.cookiecloud.get_site_cookies_file_path', lambda slug: tmp_path / f'{slug}.txt'
        ),
        patch(
            'infrastructure.site_catalog.cookiecloud.write_cookie_text_file',
...
def test_fetch_cookiecloud_cookie_data_handles_response_text_decode_failure():
    ...
    with (
        patch.object(
            ...
            return_value=('https://cookiecloud.example.com', 'uuid-1', 'password-1'),
        ),
        ...
        patch(
            'infrastructure.site_catalog.cookiecloud.decrypt',
            lambda encrypted, key: json.dumps(fake_cookie_payload).encode('utf-8'),
        ),
    ):
