...
def test_inline_video_extraction_does_not_schedule_total_video_backfill():
    with patch(
        ...
        side_effect=lambda subscription_id: (_ for _ in ()).throw(
            AssertionError('inline extraction should not schedule backfill')
        ),
    ):
...
def test_execute_full_sync_with_more_batches_continues_without_marking_success():
    ...
    with (
        patch.object(SubscriptionOrchestrator, '_has_active_subscribers', return_value=True),
        ...
        patch(
            'domains.subscription.application.services.core.update.orchestrator.SiteCatalog.is_site_enabled',
            return_value=True,
        ),
        patch(
            'domains.subscription.application.services.core.sync.lifecycle.subscription_sync_state_service.mark_sync_success',
            side_effect=lambda sync_state_id, **kwargs: success_calls.append((sync_state_id, kwargs)),
        ),
        patch(
            'domains.subscription.application.services.core.sync.lifecycle.subscription_sync_state_service.continue_full_sync_batch',
            side_effect=lambda sync_state_id, **kwargs: continuation_calls.append((sync_state_id, kwargs)),
        ),
        patch.object(
            SubscriptionSyncLifecycle,
            'request_sync',
            autospec=True,
            side_effect=lambda self, **kwargs: (
                schedule_calls.append(kwargs) or SimpleNamespace(status='queued', run_id=kwargs.get('run_id'))
            ),
        ),
...
def test_execute_incremental_schedules_full_backfill_when_observed_total_grows():
    ...
    with (
        ...
        patch(
            'domains.subscription.application.services.core.sync.lifecycle.get_subscription_by_id',
            return_value=SimpleNamespace(total_videos=10),
        ),
        patch(
            'domains.subscription.application.services.core.sync.lifecycle.subscription_sync_state_service.get_sync_state',
            return_value=SimpleNamespace(
                sync_status='success',
        ...
        patch.object(
            ...
            'request_sync',
            autospec=True,
            side_effect=lambda self, **kwargs: schedule_calls.append(kwargs) or SimpleNamespace(status='queued'),
        ),
        patch('infrastructure.database.session.get_session'),
    ):
        request = SubscriptionUpdateRequest(
