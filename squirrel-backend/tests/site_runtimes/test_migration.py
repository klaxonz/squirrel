...
from site_runtimes.migration import migrate_legacy_site_runtime_storage
from site_runtimes.paths import build_site_runtime_paths


def test_migrate_legacy_site_runtime_storage_moves_runtime_layout(tmp_path):
    ...
    migrated = migrate_legacy_site_runtime_storage(paths)
...
def test_migrate_legacy_site_runtime_storage_skips_when_new_state_exists(tmp_path):
    repo_root = tmp_path / 'repo'
    backend_root = repo_root / 'squirrel-backend'
    backend_root.mkdir(parents=True)
    paths = build_site_runtime_paths(repo_root=repo_root, backend_root=backend_root)
    ...
    paths.records_file.write_text('{}', encoding='utf-8')
