...
from schedule.tasks import thumbnail_refresh_task
from schedule.tasks.thumbnail_refresh_task import ThumbnailRefreshTask
...
def test_thumbnail_refresh_task_uses_stored_youporn_thumbnail_without_page_fetch(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(AssertionError('page fetch should not run')),
    )
    monkeypatch.setattr(
...
def test_thumbnail_refresh_task_extracts_html_escaped_og_image():
    ...
    assert ThumbnailRefreshTask._extract_thumbnail_url(html_doc) == (
...
def test_thumbnail_refresh_task_retries_page_fetch_for_retryable_status(monkeypatch):
    ...
    monkeypatch.setattr(
        thumbnail_refresh_task,
        '_get_shared_http_client',
        lambda: SimpleNamespace(get=lambda url, headers=None: responses.pop(0)),
    )
    monkeypatch.setattr(
        thumbnail_refresh_task.thumbnail_downloader_service,
