...
class SubscriptionSyncHistoryService:
    def __init__(self, session_factory=get_session, progress_service=None, run_service=None):
    ...
    def list_runs(
        ...
        *,
        ...
        base_query = self._base_run_query(user_id)
        filters = []
        ...
        if site:
            site_candidates = SubscriptionSyncHistoryService._expand_site_filter_values(site)
        ...
        parsed_from = self._parse_datetime(date_from)
        parsed_to = self._parse_datetime(date_to)
        if parsed_from:
        ...
        with self.session_factory() as session:
            if normalized_status == 'feed_recent':
                ...
                feed_completed_at_map = self._load_feed_completed_at_map(
                    session,
                ...
                total = self._count_query_rows(session, base_query)
                feed_completed_at_map = {}
        ...
        for run, subscription in rows:
            ...
            data.append({
                ...
                'site': run.site,
                'site_icon_url': self._resolve_site_icon_url(run.site),
                ...
                'queued_at': self._serialize_datetime(run.queued_at),
                ...
                'feed_completed_at': self._serialize_datetime(feed_completed_at_map.get(run.run_id)),
                'last_event_at': self._serialize_datetime(run.last_event_at),
            })
    ...
    def get_run_detail(self, run_id: str, user_id: int) -> dict | None:
        with self.session_factory() as session:
            row = session.execute(
                self._base_run_query(user_id).where(SubscriptionSyncRunProjection.run_id == run_id)
            ).first()
            if not row:
                return None
            run, subscription = row
            source_video_count = self._payload_metric_value(session, run.run_id, 'source_video_count')
            progress_snapshot = self.progress_service.build_progress_snapshot(
                status=run.status,
                current_phase=run.current_phase,
                videos_found=run.videos_found,
    ...
    def list_run_events(self, run_id: str, user_id: int) -> list[dict]:
        with self.session_factory() as session:
            if not self._run_exists_for_user(session, run_id, user_id):
                return []
        ...
        for event in events:
            data.append({
                ...
                'projected_at': self._serialize_datetime(event.projected_at),
            })
        return data

    @staticmethod
    def _expand_site_filter_values(site: str) -> list[str]:
        from utils.site_catalog import SiteCatalog
        return SiteCatalog.expand_site_filter_values(site)


_default = SubscriptionSyncHistoryService()
list_runs = _default.list_runs
get_run_detail = _default.get_run_detail
get_run_detail_snapshot = _default.get_run_detail_snapshot
list_run_events = _default.list_run_events
