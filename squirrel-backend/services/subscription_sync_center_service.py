...
from services.subscription_sync_center_queries import _default as _default_queries_service
from services.subscription_sync_progress import ACTIVE_EXTRACTION_PHASES
from services.subscription_sync_progress import _default as _default_progress_service
...
class SubscriptionSyncCenterService:
    def __init__(self, session_factory=get_session, queries_service=None, progress_service=None):
        self.session_factory = session_factory
        self.queries_service = queries_service or _default_queries_service
        self.progress_service = progress_service or _default_progress_service
    ...
    def _resolve_display_status(current_status: str | None, next_sync_at: datetime | None) -> str:
        ...
        if status == 'deferred':
            return 'deferred'
        if next_sync_at and now <= next_sync_at <= now + _default_queries_service.DUE_SOON_WINDOW:
            return 'scheduled'
        return 'healthy'
    ...
    def _load_projection_items(
        ...
        *,
        ...
        rows = self.queries_service._load_projection_rows(
            session,
            user_id=user_id,
        ...
        return [
            self._build_sync_center_item(subscription, subscription_projection, run_projection)
    ...
    def _build_sync_center_item(
        ...
        display_status = self._resolve_display_status(current_status, next_sync_at)
        ...
        return SyncCenterItemDto(
            ...
            site_icon_url=self._resolve_site_icon_url(run_projection.site if run_projection else None),
            sync_mode=sync_mode,
            ...
            last_error_summary=self._summarize_error(last_error),
    ...
    def _load_queued_task_rank_map(self, user_id: int, items: list[SyncCenterItemDto]) -> tuple[dict[int, int], dict[int, int]]:
        try:
            with self.session_factory() as session:
                return self._query_queued_task_rank_map(session, user_id, items)
    ...
    def _serialize_feed_recent_run(
        ...
        return {
            ...
            'site_icon_url': self._resolve_site_icon_url(run_projection.site),
    ...
    def get_feed_dashboard_snapshot(
        ...
        current_status = self.queries_service._projection_status_expr()
        current_phase = self.queries_service._projection_phase_expr()
        pending_videos = self.queries_service._projection_pending_videos_expr()
        now = datetime.now()
        ...
        with self.session_factory() as session:
            overview_row = session.execute(
                ...
                .where(*self.queries_service._projection_filter_clauses(site_candidates=site_candidates, normalized_query=normalized_query)),
            ).one()
            ...
            try:
                queued_candidate_rank_map, queued_backlog_rank_map = self._query_queued_task_rank_map(
                    session,
                    user_id,
                    queued_preview,
                )
            ...
            queued_preview = self._sort_items(
                queued_preview,
                'queued',
            ...
            feed_completed_at_map = self.queries_service._load_feed_completed_at_map(
                session,
...
_default = SubscriptionSyncCenterService()
get_feed_dashboard_snapshot = _default.get_feed_dashboard_snapshot
