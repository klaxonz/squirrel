...
from services.video_list_query_service import (
    _build_list_query as _default_build_list_query,
)
from services.video_list_query_service import (
    _fetch_feed_page_video_ids as _default_fetch_feed_page_video_ids,
)
...
class VideoListService:
    ...
    def list_videos(
        ...
        with self._session_factory() as session:
            ...
            thumbnail_map = self._thumbnail_downloader.get_thumbnail_url_map(
                [
            ...
            for video_id in video_ids:
                ...
                video_data = {
                    ...
                    "subscriptions": self._merge_profiles(
                        subscriptions_map.get(video.id, []),
                        self._video_extra_profiles(video, "subscriptions"),
                    ),
                    "actors": self._merge_profiles(
                        actors_map.get(video.id, []),
                        self._video_extra_profiles(video, "actors"),
                    ),
                }
    ...
    def get_video(self, user_id: int, video_id: int) -> dict[str, Any] | None:
        with self._session_factory() as session:
            ...
            video_data = {
                **video.to_dict(),
                "thumbnail": self._thumbnail_downloader.get_thumbnail_url(video.id, video.thumbnail, video.url),
                ...
                "subscriptions": self._merge_profiles(subscriptions_data, self._video_extra_profiles(video, "subscriptions")),
                "actors": self._merge_profiles(
                    [creator.to_dict() for creator in video.creators],
                    self._video_extra_profiles(video, "actors"),
                ),
                "creators": [creator.to_dict() for creator in video.creators],
                "clip_markers": [self._serialize_marker(marker) for marker in clip_markers],
            }
...
_default = VideoListService()
list_videos = _default.list_videos
get_video = _default.get_video
_merge_profiles = _default._merge_profiles
_video_extra_profiles = _default._video_extra_profiles
