...
from services.subscription.sync.constants import DUE_SOON_WINDOW
from services.subscription.sync.progress import ACTIVE_EXTRACTION_PHASES
...
def projection_pending_videos_expr() -> Any:
    return func.coalesce(
...
def base_projection_query(user_id: int) -> Any:
    return (
...
def apply_projection_filters(
    ...
    *,
    ...
    clauses = projection_filter_clauses(
        status=status,
...
def apply_projection_ordering(query: Any, status: str | None) -> Any:
    normalized_status = (status or '').strip().lower() or None
    if normalized_status == 'running':
...
def load_projection_rows(
    ...
    *,
    ...
    filtered_query = apply_projection_filters(
        base_projection_query(user_id),
        status=filter_status,
        site_candidates=site_candidates,
        normalized_query=normalized_query,
    )
    ordered_query = apply_projection_ordering(
        filtered_query,
        order_status if order_status is not None else filter_status,
    )
