...
class SiteCatalogService:
    ...
    def save_site_overrides(overrides: dict[str, dict]) -> dict[str, dict]:
        ...
        for raw_slug, raw in overrides.items():
            ...
            normalized_patch = SiteCatalogService._normalize_override_entry(slug, raw)
            desired_effective = SiteCatalogService._deep_merge_dicts(
                current_effective.get(slug, plugin_catalog[slug]), normalized_patch,
            )
            normalized_entry = SiteCatalogService._compute_override_diff(plugin_catalog[slug], desired_effective)
            if normalized_entry:
    ...
    def _normalize_override_entry(slug: str, raw: Any) -> dict[str, Any]:
        ...
        if 'enabled' in raw:
            site_entry['enabled'] = SiteCatalogService._parse_bool(raw.get('enabled', True))
        if 'label' in raw:
            label = str(raw.get('label') or '').strip()
            if label:
                site_entry['label'] = label
        if 'aliases' in raw:
            site_entry['aliases'] = SiteCatalogService._normalize_list(raw.get('aliases'), f'{slug} aliases')
        if 'http' in raw:
            ...
            headers = SiteCatalogService._normalize_headers(http_section.get('headers'), f'{slug} HTTP headers')
            site_entry['http'] = {'headers': headers} if headers else {}
        if 'proxy' in raw:
            site_entry['proxy'] = SiteCatalogService._normalize_proxy(raw.get('proxy'))
        if 'login' in raw:
            site_entry['login'] = SiteCatalogService._normalize_login(raw.get('login'))
        if 'rate_limit' in raw:
            site_entry['rate_limit'] = SiteCatalogService._normalize_rate_limit(raw.get('rate_limit'))
        if 'metadata' in raw:
            site_entry['metadata'] = SiteCatalogService._normalize_metadata(raw.get('metadata'))
        if 'cookie' in raw:
            site_entry['cookie'] = SiteCatalogService._normalize_cookie(raw.get('cookie'))
        if 'test_url' in raw:
    ...
    def _deep_merge_dicts(base: dict[str, Any], patch: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in patch.items():
            if isinstance(value, dict) and isinstance(result.get(key), dict):
                result[key] = SiteCatalogService._deep_merge_dicts(result.get(key, {}), value)
            else:
                result[key] = value
        return result
    ...
    def _compute_override_diff(base: dict[str, Any], desired: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, desired_value in desired.items():
            ...
            if isinstance(desired_value, dict) and isinstance(base_value, dict):
                nested = SiteCatalogService._compute_override_diff(base_value, desired_value)
                if nested:
                    override[key] = nested
                continue
