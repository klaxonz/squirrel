...
from sqlalchemy import and_, exists, false, func, literal, select
...
class VideoListQueryService:
    def __init__(
        ...
        if parse_search_query is None:
            from services.search_query import parse_search_query as _psq
            self._parse_search_query = _psq
        else:
            self._parse_search_query = parse_search_query

        if normalize_subscription_type_term is None:
            from services.search_query import normalize_subscription_type_term as _nstt
    ...
    def _title_search_conditions(parsed_query: Any, *, domain_column: Any = Video.domain) -> list[Any]:
        ...
        for term in parsed_query.text_terms:
            conditions.append(VideoListQueryService._contains(Video.title, term))

        for term in parsed_query.get("title"):
            conditions.append(VideoListQueryService._contains(Video.title, term))

        for term in parsed_query.get("domain"):
            conditions.append(VideoListQueryService._contains(domain_column, term))
    ...
    def _subscription_search_query(
        ...
        *,
        ...
        for term in subscription_terms + text_terms:
            subscription_query = subscription_query.where(self._contains(active_subscriptions.c.subscription_name, term))
        ...
        for cond in self._feed_time_range_predicates(time_range):
...
_default = VideoListQueryService()
_build_feed_rows_query = _default._build_feed_rows_query
_build_list_query = _default._build_list_query
_fetch_feed_page_video_ids = _default._fetch_feed_page_video_ids
