"""Backward-compatible music service facade.

The implementation lives in :mod:`services.music` (split by domain). This
module re-exports the public surface so existing callers such as
``routes.music`` and ``tests/services/test_music_service.py`` keep
working with ``from services import music_service``.

The test suite monkeypatches ``music_service.settings``,
``music_service.redis_client``, and ``music_service.httpx``. For that to
propagate to the implementation, the submodules read these through this
module at call time — see ``services.music._client``.
"""
