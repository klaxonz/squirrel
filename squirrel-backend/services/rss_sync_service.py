...
class RssSyncService:
    ...
    def sync_account(self, user_id: int, account_id: int, *, entry_limit: int | None = None, force_full_sync: bool = False) -> dict[str, Any] | None:
        ...
        try:
            ...
            with self.session_factory() as session:
                ...
                feeds = self._upsert_feeds(session, account, remote_feeds)
            ...
            if isinstance(client, GReaderClient):
                ...
                with self.session_factory() as session:
                    ...
                    for page in client.iter_recent_entries(greader_entry_limit, _on_entries_fetched):
                        new_feeds = self._load_feeds_by_external_id(session, account_id, page, feeds_by_external_id)
                        feeds_by_external_id.update(new_feeds)
                        batch_entries = self._upsert_remote_entries_batch(session, feeds_by_external_id, page)
                ...
                with self.session_factory() as session:
                    for feed_id, external_feed_id, enabled in feed_refs:
                        ...
                        batch_entries = self._upsert_remote_entries_batch(session, feeds_by_external_id, remote_entries)
                        synced_entries += batch_entries
    ...
    def sync_feed(self, user_id: int, feed_id: int, *, entry_limit: int = 50) -> dict[str, Any]:
        with self.session_factory() as session:
            ...
            feeds_by_external_id = {external_feed_id: feed}
            synced = self._upsert_remote_entries_batch(session, feeds_by_external_id, remote_entries)
            feed.last_entry_sync_at = datetime.now()
            feed.enabled = True
            account.last_error = None
            session.commit()
...
_default = RssSyncService()
sync_account = _default.sync_account
subscribe_feed = _default.subscribe_feed
sync_feed = _default.sync_feed
unsubscribe_feed = _default.unsubscribe_feed
