...
from sqlalchemy import JSON, VARCHAR, Boolean, DateTime, Index, Integer, Text, UniqueConstraint
...
class RssAccount(Base, SerializerMixin):
    ...
    sync_entry_limit: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class RssFeed(Base, SerializerMixin):
    ...
    feed_url: Mapped[str | None] = mapped_column(VARCHAR(2048), nullable=True)
    site_url: Mapped[str | None] = mapped_column(VARCHAR(2048), nullable=True)
    icon_url: Mapped[str | None] = mapped_column(VARCHAR(2048), nullable=True)
    category: Mapped[str | None] = mapped_column(VARCHAR(255), nullable=True)
    enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    open_method: Mapped[str | None] = mapped_column(VARCHAR(32), nullable=True, default=None)
    raw_data: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    last_entry_sync_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    created_at: Mapped[datetime] = mapped_column(default=lambda: datetime.now())
    updated_at: Mapped[datetime] = mapped_column(default=lambda: datetime.now(), onupdate=lambda: datetime.now())
