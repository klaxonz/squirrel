...
class ScheduledTask(Base, SerializerMixin):
    ...
    # 执行信息
    last_run_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, comment="最后执行时间")
    ...
    created_by: Mapped[Optional[str]] = mapped_column(VARCHAR(100), nullable=True, comment="创建者")
    updated_by: Mapped[Optional[str]] = mapped_column(VARCHAR(100), nullable=True, comment="更新者")
...
class TaskExecutionLog(Base, SerializerMixin):
    ...
    result_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True, comment="执行结果数据")

    # 元数据
    executed_by: Mapped[Optional[str]] = mapped_column(VARCHAR(100), nullable=True, comment="执行者")
    trace_id: Mapped[Optional[str]] = mapped_column(VARCHAR(100), nullable=True, comment="链路追踪ID")
