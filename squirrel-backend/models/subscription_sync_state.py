...
class SubscriptionSyncState(Base, SerializerMixin):
    ...
    last_head_sample_urls: Mapped[Optional[list[str]]] = mapped_column(JSON, nullable=True, default=list)
    last_head_fingerprint: Mapped[Optional[str]] = mapped_column(VARCHAR(64), nullable=True)
    last_known_total_available: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    gap_suspicion_score: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    gap_suspicion_reason: Mapped[Optional[str]] = mapped_column(TEXT, nullable=True)
    last_gap_detected_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    last_full_requested_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
