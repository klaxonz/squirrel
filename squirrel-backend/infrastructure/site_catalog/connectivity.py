        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                if 200 <= response.status_code < 400:
                    ...
                    logger.info(
                        "Site %s is accessible, status: %s, time: %f'.2f'ms", url, response.status_code, response_time
                    )
                    ...
                    logger.info(
                        'Site %s responded with restricted status %s but is reachable', url, response.status_code
                    )
                else:
