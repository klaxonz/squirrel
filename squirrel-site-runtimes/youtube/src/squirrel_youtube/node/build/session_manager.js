...
class ProxySpec {
  ...
  asDispatcher(logger) {
    ...
    const proxyStr = this.proxy;
    const { password } = proxyUrl;
    const loggedProxy = password ? proxyStr.replace(password, '****') : proxyStr;
    logger.log(`Using proxy: ${loggedProxy}`);
    try {
      return new ProxyAgent({
        getProxyForUrl: () => proxyStr,
        localAddress: sourceAddress,
        family: this.ipFamily,
        rejectUnauthorized: !disableTlsVerification,
      });
...
class CacheSpec {
  constructor(proxySpec, ip) {
    this.proxySpec = proxySpec;
    this.ip = ip;
  }

  get key() {
    return JSON.stringify(this.ip || [this.proxySpec.proxy, this.proxySpec.sourceAddress]);
  }
}
...
export class SessionManager {
  ...
  constructor(shouldLog = true, youtubeSessionDataCaches) {
    ...
    this.TOKEN_TTL_HOURS = process.env.TOKEN_TTL ? parseInt(process.env.TOKEN_TTL) : 6;
  ...
  async getDescrambledChallenge(bgConfig, challenge, innertubeContext) {
    try {
      ...
      throw new Error('Could not get BotGuard challenge', { cause: err });
    }
  }
  ...
  async generateTokenMinter(cacheSpec, bgConfig, challenge, innertubeContext) {
    ...
    const interpreterJavascript =
      descrambledChallenge.interpreterJavascript.privateDoNotAccessOrElseSafeScriptWrappedValue;
    ...
    try {
      ...
      throw new Error('Failed to create BG client.', { cause: err });
    }
    ...
    try {
      ...
      const integrityTokenResp = await bgConfig.fetch(buildURL('GenerateIT'), {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify([SessionManager.REQUEST_KEY, botguardResponse]),
      });
      const [
        integrityToken,
        estimatedTtlSecs,
      ...
      throw new Error('Failed to generate an integrity token.', { cause: err });
    }
  }
  ...
  async tryMintPOT(contentBinding, tokenMinter) {
    ...
    try {
      ...
      throw new Error(`Failed to mint POT for ${contentBinding}: ${err.message}`, { cause: err });
    }
  }
      ...
      for (let attempts = 1; attempts <= maxRetries; attempts++) {
        try {
          ...
          const response = await (
            method === 'GET' ? axios.get(url, axiosOpt) : axios.post(url, options?.body, axiosOpt)
          );
          return {
          ...
          if (attempts >= maxRetries) {
            throw new Error(`Error reaching ${method} ${url}: All ${attempts} retries failed.`, {
              cause: err,
            });
          }
  ...
  async generatePoToken(
    ...
    this.cleanupCaches();
    const proxySpec = new ProxySpec({
      sourceAddress,
      disableTlsVerification,
    });
    if (proxy) {
      proxySpec.proxy = proxy;
    } else {
      proxySpec.proxy = process.env.HTTPS_PROXY || process.env.HTTP_PROXY || process.env.ALL_PROXY;
    }
    const cacheSpec = new CacheSpec(proxySpec, innertubeContext?.client.remoteHost || null);
    const bgFetch = this.getFetch(proxySpec, 3, 5000);
