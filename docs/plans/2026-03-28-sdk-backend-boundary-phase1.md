...
def test_filter_cookies_to_query_string_by_domain_uses_backend_resolver(monkeypatch):
    monkeypatch.setattr(cookie, 'resolve_cookie_file_for_url', lambda url: 'D:/tmp/mock.txt')
    monkeypatch.setattr(cookie, '_read_cookie_file_as_query_string', lambda path: 'a=1; b=2')
